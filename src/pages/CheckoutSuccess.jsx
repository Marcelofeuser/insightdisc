...
export default function CheckoutSuccess() {
  ...
  useEffect(() => {
    const processCheckout = async () => {
      ...
      setFlow(fallbackFlow || (fallbackToken || fallbackEmail || fallbackName ? 'candidate' : ''));
      ...
      if ((isMock || isDev) && fallbackAssessmentId) {
        ...
        try {
          ...
          // fallback local: continue flow even if update fails in mock/dev
        } finally {
          setError('');
          setIsLoading(false);
        }
      ...
      try {
        ...
        setFlow(data.flow || fallbackFlow || (data.token || fallbackToken ? 'candidate' : ''));
        ...
        if (isMock && fallbackAssessmentId) {
          try {
            ...
            setAssessmentId(fallbackAssessmentId);
            setCandidateToken(fallbackToken || '');
            setFlow(fallbackFlow || (fallbackToken ? 'candidate' : ''));
            setError('');
          } catch {
            setAssessmentId(fallbackAssessmentId);
            setPublicToken('');
        ...
        <CardContent className="p-8 space-y-5">
          {isLoading ? (
            ...
            <>
              ...
              <h1 className="text-2xl font-bold text-slate-900">Relatório liberado</h1>
